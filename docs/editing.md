...
The `add` payload is the filled-in template from
[`check --out`](translating.md). Beyond plain `"key": "value"` pairs it takes a
per-key `state` and `comment`, and a `plural` object for variations. `"TODO"`
values are left alone, so a template can be filled in over several passes; a
bare `{"key": "value"}` object also works, with `--lang` and the catalog from
the command line; `-` reads from stdin.
