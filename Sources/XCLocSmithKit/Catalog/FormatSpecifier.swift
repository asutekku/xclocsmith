...
public enum FormatSpecifierScanner {
    /// Matches printf-style specifiers, `%%` escapes, and `%#@name@`
    ...
    private static let pattern = try! NSRegularExpression(
        pattern: #"%(%|#@[A-Za-z_][A-Za-z0-9_]*@|(?:(\d+)\$)?[-+#0']*\d*(?:\.\d+)?(hh|h|ll|l|q|z|t|j|L)?([@dDiuUxXoOfFeEgGaAcCsS]))"#
    )
    ...
    /// sentences.
    public static func mismatch(source: String, translation: String) -> String? {
        let sourceSpecifiers = specifiers(in: source)
        let translationSpecifiers = specifiers(in: translation)
